...
namespace GoProMP4MetadataExtraction
{
    ...
    public class GpmfItemList : List<GpmfItem>
	{
		...
		private static void GetItems(GpmfItemList searchItems, string fourcc, GpmfItemList foundItems)
		{
            foreach (GpmfItem item in searchItems)
			{
				...
				if (item.TypeSize.Type == 0 && item.Payload is not null)
				{
                    GpmfItemList searchItemsChild = (GpmfItemList)item.Payload;

                    GetItems(searchItemsChild, fourcc, foundItems);
				}
			}
		}
	}
